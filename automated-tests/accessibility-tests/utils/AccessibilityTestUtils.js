  ...
  static async validateFormAccessibility(page, formSelector = 'form') {
    ...
    for (let formIndex = 0; formIndex < forms.length; formIndex++) {
      ...
      for (let inputIndex = 0; inputIndex < inputs.length; inputIndex++) {
        ...
        if (
          !inputResult.hasAccessibleLabel &&
          inputInfo.type !== 'hidden' &&
          inputInfo.type !== 'submit'
        ) {
          formResult.issues.push(
            `Input ${inputInfo.name || inputInfo.id || inputIndex} lacks accessible label`
          );
        }

        if (inputInfo.required && !inputInfo.ariaRequired) {
          formResult.issues.push(
            `Required input ${inputInfo.name || inputInfo.id || inputIndex} should have aria-required="true"`
          );
        }
      }
    ...
    for (let i = 0; i < headingStructure.length; i++) {
      ...
      if (i > 0) {
        ...
        if (current.level > previous.level + 1) {
          issues.push(
            `Heading level skip: h${previous.level} to h${current.level} at "${current.text}"`
          );
        }
      }
    }
