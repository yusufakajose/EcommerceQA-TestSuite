...
class ApiPerformanceMonitor {
  ...
  async measureApiCall(apiCallFunction, operationName, threshold = 1000) {
    ...
    try {
      ...
      if (duration > threshold) {
        console.warn(
          `⚠️ Performance Warning: ${operationName} took ${duration}ms (threshold: ${threshold}ms)`
        );
      }
  ...
  analyzeSizePerformanceCorrelation() {
    ...
    const avgBytesPerMs =
      measurementsWithSize.reduce((sum, m) => sum + m.bytesPerMs, 0) / measurementsWithSize.length;
  ...
  analyzePerformanceTrends() {
    ...
    const earlierAvg =
      earlierMeasurements.reduce((sum, m) => sum + m.duration, 0) / earlierMeasurements.length;
