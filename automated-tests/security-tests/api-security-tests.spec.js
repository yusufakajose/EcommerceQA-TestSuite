...
test.describe('API Security Tests', () => {
  ...
  test.describe('API Input Validation', () => {
    test('API XSS Prevention', async ({ page }) => {
      ...
      for (const endpoint of testEndpoints) {
        for (const payload of xssPayloads) {
          ...
          const containsUnescapedPayload =
    ...
    test('API SQL Injection Prevention', async ({ page }) => {
      ...
      const testEndpoints = ['/api/users/1', '/api/products/search?q=', '/api/orders?user_id='];
  ...
  test.describe('API Data Exposure', () => {
    test('Sensitive Data Exposure in API Responses', async ({ page }) => {
      const testEndpoints = ['/api/users', '/api/users/profile', '/api/admin/users'];
    ...
    test('API Error Message Information Disclosure', async ({ page }) => {
      const testEndpoints = ['/api/nonexistent', '/api/users/999999', '/api/admin/secret'];
  ...
  test.describe('API Security Headers', () => {
    test('API CORS Configuration', async ({ page }) => {
      ...
      if (allowMethods) {
        ...
        const allowsDangerous = dangerousMethods.some((method) =>
          allowMethods.toUpperCase().includes(method)
        );
