...
class SecurityTestUtils {
  ...
  static async analyzeSessionSecurity(page) {
    ...
    const totalIssues = analysis.securityIssues.reduce(
      (sum, cookie) => sum + cookie.issues.length,
      0
    );
    if (totalIssues > 0) {
  ...
  static async testFileUploadSecurity(page, fileInputSelector) {
    ...
    if (acceptAttribute) {
      ...
      analysis.allowsExecutables = dangerousTypes.some((type) =>
        acceptAttribute.toLowerCase().includes(type)
      );
    } else {
  ...
  static async testCSRFProtection(page, formSelector = 'form') {
    ...
    for (let i = 0; i < forms.length; i++) {
      ...
      if (
        formAnalysis.hasCSRFToken &&
        formAnalysis.tokenValue &&
        formAnalysis.tokenValue.length < 16
      ) {
    ...
    return {
      ...
      summary: {
        ...
        postFormsWithoutCSRF: results.filter(
          (r) => !r.hasCSRFToken && r.method.toLowerCase() === 'post'
        ).length,
      },
    };
  }
