...
test.describe('Shopping Cart Tests', () => {
  ...
  test.describe('Add to Cart Functionality', () => {
    test('should add single product to cart', async ({ page }) => {
      ...
      // Verify cart totals
      await expect(cartPage.getSubtotal()).toContainText(
        singleItemScenario.expectedTotal.toString()
      );
      await expect(cartPage.getTotalItemCount()).toContainText(
        singleItemScenario.expectedItemCount.toString()
      );
    });
    ...
    test('should add multiple different products to cart', async ({ page }) => {
      ...
      await expect(catalogPage.getCartItemCount()).toContainText(
        multipleItemsScenario.expectedItemCount.toString()
      );
      ...
      for (const item of multipleItemsScenario.items) {
        ...
        await expect(cartPage.getProductQuantity(item.productId)).toHaveValue(
          item.quantity.toString()
        );
        await expect(cartPage.getProductSubtotal(item.productId)).toContainText(
          item.expectedSubtotal.toString()
        );
      }

      // Verify cart totals
      await expect(cartPage.getSubtotal()).toContainText(
        multipleItemsScenario.expectedTotal.toString()
      );
    });
    ...
    test('should handle out of stock products', async ({ page }) => {
      ...
      const outOfStockMessage = await catalogPage
        .getOutOfStockMessage(outOfStockProduct.id)
        .isVisible()
        .catch(() => false);
      ...
      if (!isDisabled) {
        ...
        await expect(catalogPage.getErrorMessage()).toContainText(
          /Out of stock|Not available|Cannot add/
        );
      }
    });
    ...
    test('should respect maximum quantity limits', async ({ page }) => {
      ...
      await catalogPage.addProductToCart(
        maxQuantityTest.productId,
        maxQuantityTest.attemptedQuantity
      );

      // Should show error or limit quantity
      const errorMessage = await catalogPage
        .getErrorMessage()
        .textContent()
        .catch(() => '');
      if (errorMessage.includes('Maximum') || errorMessage.includes('limit')) {
        ...
        await cartPage.navigate();
        const actualQuantity = await cartPage
          .getProductQuantity(maxQuantityTest.productId)
          .inputValue();
  ...
  test.describe('Update Cart Functionality', () => {
    test('should update product quantity in cart', async ({ page }) => {
      ...
      await expect(cartPage.getProductQuantity(update.productId)).toHaveValue(
        update.newQuantity.toString()
      );
      await expect(cartPage.getProductSubtotal(update.productId)).toContainText(
        update.expectedSubtotal.toString()
      );
      await expect(cartPage.getSubtotal()).toContainText(
        quantityUpdateScenario.expectedTotal.toString()
      );
    });
  ...
  test.describe('Remove from Cart Functionality', () => {
    test('should remove single product from cart', async ({ page }) => {
      ...
      await expect(cartPage.getCartItems()).toHaveCount(
        removeItemsScenario.expectedRemainingItems.length
      );
      ...
      await expect(cartPage.getSubtotal()).toContainText(
        removeItemsScenario.expectedTotal.toString()
      );
    });
    ...
    test('should clear entire cart', async ({ page }) => {
      ...
      await expect(cartPage.getSubtotal()).toContainText(
        emptyCartScenario.expectedTotal.toString()
      );
    });
  ...
  test.describe('Cart Calculations', () => {
    ...
    test('should calculate tax correctly', async ({ page }) => {
      const taxScenario = testDataHelper
        .getTestData('cart', 'cartCalculations')
    ...
    test('should calculate shipping correctly', async ({ page }) => {
      const shippingScenario = testDataHelper
        .getTestData('cart', 'cartCalculations')
      ...
      await expect(cartPage.getShipping()).toContainText(
        shippingScenario.expectedShipping.toString()
      );
    ...
    test('should apply free shipping threshold', async ({ page }) => {
      const freeShippingScenario = testDataHelper
        .getTestData('cart', 'cartCalculations')
      ...
      await expect(cartPage.getTotal()).toContainText(
        freeShippingScenario.expectedTotal.toString()
      );
    ...
    test('should show free shipping progress', async ({ page }) => {
      ...
      if (isVisible) {
        ...
        await expect(progressIndicator).toContainText(
          /Add.*more.*free shipping|.*away from free shipping/
        );
      }
    });
  });
  ...
  test.describe('Cart Persistence and State', () => {
    ...
    test('should persist cart across browser sessions', async ({ page, context }) => {
      ...
      const cartCount = await newCatalogPage
        .getCartItemCount()
        .textContent()
        .catch(() => '0');
      if (cartCount !== '0') {
  ...
  test.describe('Cart UI and UX', () => {
    ...
    test('should show cart item count in navigation', async ({ page }) => {
      ...
      const initialCount = await catalogPage
        .getCartItemCount()
        .textContent()
        .catch(() => '0');
