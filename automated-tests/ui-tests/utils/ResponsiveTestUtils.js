...
class ResponsiveTestUtils {
  ...
  async doesElementOverflowHorizontally(element) {
    ...
    return elementBox.width > viewport.width || elementBox.x + elementBox.width > viewport.width;
  }
  ...
  async getElementResponsiveBehavior(element, viewports) {
    ...
    for (const viewport of viewports) {
      ...
      behavior[viewport.name] = {
        ...
        overflowsHorizontally: isVisible
          ? await this.doesElementOverflowHorizontally(element)
          : false,
      };
    }
  ...
  async getResponsiveTestReport(element) {
    ...
    for (const viewport of viewports) {
      ...
      if (isVisible) {
        ...
        if (overflowsHorizontally) report.summary.overflowingViewports++;
        if (touchInfo.isTouchFriendly) report.summary.touchFriendlyViewports++;
      }
