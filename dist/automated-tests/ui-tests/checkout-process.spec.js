...
test.describe('Checkout Process Tests', () => {
    ...
    test.describe('Checkout Flow - Logged In User', () => {
        test('should complete successful checkout with valid data', async ({ page }) => {
            ...
            await expect(checkoutPage.getSuccessMessage()).toContainText(checkoutScenario.expectedSuccessMessage);
            // Verify order confirmation details
            await expect(checkoutPage.getOrderNumber()).toBeVisible();
            await expect(checkoutPage.getOrderTotal()).toContainText(checkoutScenario.expectedOrderTotal.toString());
        });
        ...
        test('should use same address for billing and shipping', async ({ page }) => {
            ...
            await expect(checkoutPage.getBillingField('firstName')).toHaveValue(checkoutScenario.shippingAddress.firstName);
            await expect(checkoutPage.getBillingField('street')).toHaveValue(checkoutScenario.shippingAddress.street);
            await expect(checkoutPage.getBillingField('city')).toHaveValue(checkoutScenario.shippingAddress.city);
        });
        test('should use different billing address', async ({ page }) => {
            ...
            await expect(checkoutPage.getBillingField('street')).toHaveValue(checkoutScenario.billingAddress.street);
            await expect(checkoutPage.getBillingField('city')).toHaveValue(checkoutScenario.billingAddress.city);
        });
    });
    test.describe('Checkout Flow - Guest User', () => {
        test('should complete guest checkout successfully', async ({ page }) => {
            ...
            // Verify successful order
            await expect(checkoutPage.getSuccessMessage()).toBeVisible();
            await expect(checkoutPage.getOrderTotal()).toContainText(guestCheckoutScenario.expectedOrderTotal.toString());
        });
    ...
    test.describe('Payment Method Validation', () => {
        ...
        test('should handle insufficient funds error', async ({ page }) => {
            ...
            await checkoutPage.placeOrder();
            await expect(checkoutPage.getPaymentError()).toBeVisible();
            await expect(checkoutPage.getPaymentError()).toContainText(insufficientFundsCard.expectedError);
        });
    });
    test.describe('Form Validation', () => {
        test('should validate required shipping fields', async ({ page }) => {
            const validationErrors = testDataHelper.getTestData('checkout', 'validationErrors')
        ...
        test('should validate ZIP code format', async ({ page }) => {
            const zipCodeValidation = testDataHelper.getTestData('checkout', 'validationErrors')
            ...
            await expect(checkoutPage.getFieldError('zipCode')).toContainText(zipCodeValidation.expectedErrors[0]);
        });
        test('should validate phone number format', async ({ page }) => {
            const phoneValidation = testDataHelper.getTestData('checkout', 'validationErrors')
            ...
            await expect(checkoutPage.getFieldError('phone')).toContainText(phoneValidation.expectedErrors[0]);
        });
        test('should validate credit card number format', async ({ page }) => {
            ...
            for (const invalidCard of invalidCardNumbers) {
                ...
                await expect(checkoutPage.getPaymentFieldError('cardNumber')).toContainText(/Invalid card number|Please enter a valid card number/);
            }
        });
        test('should validate expiry date', async ({ page }) => {
            ...
            await expect(checkoutPage.getPaymentFieldError('expiryYear')).toContainText(/Card has expired|Expiry date is invalid/);
        });
    });
    test.describe('Shipping Options', () => {
        test('should display available shipping options', async ({ page }) => {
            ...
            for (const option of shippingOptions) {
                ...
                await expect(checkoutPage.getShippingOptionCost(option.id)).toContainText(option.cost.toString());
            }
        });
        ...
        test('should show free shipping when eligible', async ({ page }) => {
            const freeShippingOption = testDataHelper.getTestData('checkout', 'shippingOptions')
            ...
            const freeShippingVisible = await checkoutPage.getShippingOption('free').isVisible().catch(() => false);
    ...
    test.describe('Order Review and Confirmation', () => {
        ...
        test('should show order confirmation after successful payment', async ({ page }) => {
            ...
            await expect(checkoutPage.getConfirmationShippingAddress()).toContainText(checkoutScenario.shippingAddress.street);
            await expect(checkoutPage.getConfirmationPaymentMethod()).toContainText('****' + checkoutScenario.paymentMethod.cardNumber.slice(-4));
        });
        test('should send order confirmation email', async ({ page }) => {
            ...
            await expect(checkoutPage.getEmailConfirmationMessage()).toContainText(/confirmation email|email sent|check your email/i);
        });
    });
    test.describe('Error Handling and Edge Cases', () => {
        ...
        test('should handle inventory changes during checkout', async ({ page }) => {
            ...
            const inventoryError = await checkoutPage.getInventoryError().isVisible().catch(() => false);
            if (inventoryError) {
                await expect(checkoutPage.getInventoryError()).toContainText(/insufficient inventory|not enough stock|quantity not available/i);
            }
        });
    });
});
