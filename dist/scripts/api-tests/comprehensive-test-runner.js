...
class ComprehensiveTestRunner extends APITestRunner {
    ...
    async runSecurityTests(environment, options = {}) {
        ...
        const securityResults = await this.runMultipleCollections(this.testSuites.security, environment, securityOptions);
    ...
    async runPerformanceTests(environment, options = {}) {
        ...
        const performanceResults = await this.runDataDrivenTest('user-management', environment, 'performance-test-data', 10 // 10 iterations for performance testing
        );
    ...
    analyzePerformanceResults(results) {
        ...
        if (results.requests && results.requests.length > 0) {
            ...
            if (responseTimes.length > 0) {
                analysis.averageResponseTime = Math.round(responseTimes.reduce((sum, time) => sum + time, 0) / responseTimes.length);
    ...
    async runNegativeTests(environment = 'development') {
        ...
        return this.runDataDrivenTest('enhanced-user-management', environment, 'negative-test-data', null // Use all rows in CSV
        );
    }
    /**
