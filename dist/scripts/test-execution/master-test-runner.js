...
class MasterTestRunner {
    ...
    async executeSuite(suiteId, config) {
        ...
        try {
            ...
            for (const env of environments) {
                ...
                const envResult = await this.executeSuiteForEnvironment(suiteId, env, config.browsers, config);
    ...
    getResultPaths(suiteId, environment) {
        ...
        switch (suiteId) {
            case 'ui':
                paths.push(path.join(this.baseDir, 'test-results', 'results.json'), path.join(this.baseDir, 'playwright-report', 'results.json'), path.join(this.baseDir, `reports/test-execution/${environment}/results.json`));
                break;
            case 'api':
                paths.push(path.join(this.baseDir, 'reports/api-tests', 'summary.json'), path.join(this.baseDir, 'reports/newman', 'summary.json'));
                break;
            case 'performance':
