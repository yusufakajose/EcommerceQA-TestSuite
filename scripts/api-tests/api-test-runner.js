...
class APITestRunner {
  ...
  processSummary(summary, collectionName, environmentName) {
    ...
    const result = {
      ...
      requests: executions.map(execution => ({
        ...
        assertions: execution.assertions ? execution.assertions.map(assertion => ({
  ...
  async runMultipleCollections(collections, environment = 'development', options = {}) {
    ...
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const consolidatedPath = path.join(this.resultsDir, `consolidated-${environment}-${timestamp}.json`);
  ...
  generateExecutionReport(results, outputPath = null) {
    const reportPath = outputPath || path.join(this.reportsDir, `execution-report-${Date.now()}.html`);
